...
function TinyPNG(opt, obj) {
  ...
  this.stream = function () {
    ...
    return (opt.parallel ? throughParallel : through)
      .obj({ maxConcurrency: opt.parallelMax }, function (file, enc, cb) {
        ...
        if (file.isBuffer()) {
          ...
          self.request(file).get(
            function (err, tinyFile) {
              if (err) {
        ...
        if (opt.summarize) {
          var stats = self.stats,
            info = util.format(
              ...
              stats.total.in
                ? Math.round((stats.total.out / stats.total.in) * 10000) / 10000
                : 0
            );
          ...
          if (stats.retries > 0) {
            ...
            stats.retried.forEach(function (item) {
              self.utils.log(
                item.file + ": " + item.attempts + " attempts",
                true
              );
            });
          }
  ...
  this.request = function (file, cb) {
    ...
    return {
      ...
      upload: function (cb) {
        ...
        if (file.contents <= 0) {
          err = new Error(
            "Error: Empty or broken images could not be send " + file.relative
          );
          return cb(err);
        }
            ...
            if (err) {
              ...
              if (res.statusCode == 200 || res.statusCode == 201) {
                try {
                  ...
                  err = new Error(
                    "Upload response JSON parse failed, invalid data returned from API. Failed with message: " +
                      e.message
                  );
                }
                ...
                if (!err) {
                  if (data.error) {
                    ...
                    info.url = self.conf.options.keepMetadata
                      ? res.headers.location
                      : data.output.url;
                  } else {
                    err = new Error(
                      "Invalid TinyPNG response object returned for " +
                        file.relative
                    );
                  }
                }
                ...
                err = new Error(
                  "Error: Statuscode " + res.statusCode + " returned"
                );
              }
            } else {
              err = new Error(
                "No content returned from TinyPNG API for" + file.relative
              );
            }
      ...
      download: function (url, cb) {
        ...
        if (self.conf.options.keepMetadata) {
          ...
          options.headers = {
            Authorization: useCorsApi ? undefined : "Basic " + self.conf.token,
          };
        }
        request.get(options, function (err, res, body) {
          err = err
            ? new Error(
                "Download failed for " + url + " with error: " + err.message
              )
            : false;
          var buffer = false;
          try {
            ...
            return cb(
              new Error("Empty Body for Download with error: " + err.message)
            );
          }
  ...
  this.utils = {
    ...
    prettySize: function (bytes) {
      ...
      return (
        (bytes / Math.pow(1024, pos)).toFixed(2) +
        " " +
        " KMGTP".charAt(pos) +
        "B"
      );
